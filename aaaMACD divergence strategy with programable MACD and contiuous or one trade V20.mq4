...
input string   inpTradeComments  =  "Macd divergence V20";  //Trade comment
...
double  nowminusdi ;
 ...
 Comment (" Spread =  "+MarketInfo(Symbol(),MODE_SPREAD)+ "  Minimum stop = "+  MarketInfo(Symbol(),MODE_SPREAD));
  ...
  if ((waitingForShort== true) && (countdown<4)) 
...
//////////////////===============================================================================================================================
  ...
  if ((waitingForLong==true) && (countdown<4))
   ...
   for (int t=2;t<40;t++)
         {
                   ...
                   priceExtreme=false;
                    ...
                    // if (iMACD(Symbol(),higherConfirm,12,26,9,PRICE_CLOSE,MODE_MAIN,1)<iMACD(Symbol(),higherConfirm,12,26,9,PRICE_CLOSE,MODE_MAIN,2))momconfirm=1;
                     // else momconfirm=-1;
                     
                     double PriceHigh =iHigh (Symbol(),Period(),higherHigh);
                    if (PriceHigh>KeltnerUpperBand) priceExtreme= true;
                    
                    if ((bar1==1)&& (higherHigh==1)&&(MACDLow>MACDHigh) && (goShort==true) &&  (notrade==false)&& (notDiverging==false) && 
                    (adxtrigger<inpasxCrosstrigger) && (priceExtreme==true)) 
                              {
                              divergence=true; waitingForShort=true;countdown=0;
                              }
                    
                      
                        if ((MACDLow<MACDHigh) && (goShort==true))
                        divergence=false;
                     ...
                     if ((xbar1==1)&& (lowerLow==1)&&(MACDLower>MACDHigher) && (goLong==true)&& (notrade==false)&& (notDiverging==false)&& (adxtrigger<inpasxCrosstrigger)&& (priceExtreme==true))
                     
                        {divergence=true; countdown=0; waitingForLong=true;} 
