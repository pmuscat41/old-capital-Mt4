...
//
//
// v75 - no direction for trades- loss adjusted
...
input double    lotsWinning=0.2;// lots if last trade was a win
input double    lotsLosses= 0.001;// lots if last trade was a loss
...
Docket= CheckLongTrade (higherHigh,lowerLow,multiplier,XmarketTurnCounter);}
 ...
 /////-------------------------------------------------------------------------------------------------------------------------------------
 if (   (stomain>stosig)&& (marketTurnCounter< inpMarketTurnCounter)&& (completed==false)) 
 ...
 
 {
        ...
        double stosig=iStochastic(Symbol(),Period(),3,3,14,MODE_SMA,0,MODE_SIGNAL,1);
          ...
          if ( (stomain>stosig) && (XmarketTurnCounter< inpMarketTurnCounter)&&(completed==false)) 
                 {
