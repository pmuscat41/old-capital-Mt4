...
input string   inpTradeComments  =  "Hedging EA v9";  //Trade comment
...
void OnTick()
  {
   ...
   if(!newBar())
    ...
    //  if (trades<inpMaxOrders)
     //    if (buys<sells || buys==0) {ticket   =  orderOpen(ORDER_TYPE_BUY, stploss,takeProfit);
                 //          }
      
      
    
   //   if (trades<inpMaxOrders)
     //    if (sells<buys || sells==0) {ticket  =  orderOpen(ORDER_TYPE_SELL, stploss,takeProfit);
