...
input ENUM_TIMEFRAMES  inpStopTrailTimeframe=PERIOD_M1;// Timeframe to update trailing stop
                  ...
                  if (Buy50Stop==true)  
                     //if (buyStopLoss>OrderOpenPrice())
                        if (buyStopLoss>OrderStopLoss())
                           ...
                           Debug("Buystop50 true - buystoploss is ",buyStopLoss);}
                            ...
                            if (Sell50Stop==true)
                               //  if  (sellStopLoss<OrderOpenPrice())
                                    if (sellStopLoss<OrderStopLoss())
                                     ...
                                     Debug("Sell50 true - sellStoploss is ",sellStopLoss);}
               ...
               if (OrderType()==ORDER_TYPE_BUY)
                  if (Buy60Stop==true)
                     //if (buyStopLoss>OrderOpenPrice())
                       if (buyStopLoss>OrderStopLoss())
                           {OrderModify(OrderTicket(), OrderOpenPrice(),buyStopLoss,OrderTakeProfit(),OrderExpiration());
                           Debug("Buystop60 true - buystoploss is ",buyStopLoss);}
                        ...
                        if (OrderMagicNumber()==magicNumber)
                          if (OrderType()==ORDER_TYPE_SELL)
                           ...
                           // if  (sellStopLoss<OrderOpenPrice())
                             if (sellStopLoss<OrderStopLoss())
                                 {OrderModify(OrderTicket(), OrderOpenPrice(),sellStopLoss,OrderTakeProfit(),OrderExpiration());
                                 Debug("Sell60 true - sellStoploss is ",sellStopLoss);}
            ...
            if (OrderMagicNumber()==magicNumber)
               if (OrderType()==ORDER_TYPE_BUY)
                 ...
                 // if (buyStopLoss>OrderOpenPrice())
                   if (buyStopLoss>OrderStopLoss())
                          { OrderModify(OrderTicket(), OrderOpenPrice(),buyStopLoss,OrderTakeProfit(),OrderExpiration());
                           Debug("Buystop70 true - buystoploss is ",buyStopLoss);}
                        ...
                        if (OrderMagicNumber()==magicNumber)
                          if (OrderType()==ORDER_TYPE_SELL)
                            ...
                            //if  (sellStopLoss<OrderOpenPrice())
                             if (sellStopLoss<OrderStopLoss())
                                 {OrderModify(OrderTicket(), OrderOpenPrice(),sellStopLoss,OrderTakeProfit(),OrderExpiration());
                                 Debug("Sell70 true - sellStoploss is ",sellStopLoss);}
            ...
            if (OrderMagicNumber()==magicNumber)
               if (OrderType()==ORDER_TYPE_BUY)
                 ...
                 // if (buyStopLoss>OrderOpenPrice())
                   if (buyStopLoss>OrderStopLoss())
                           {OrderModify(OrderTicket(), OrderOpenPrice(),buyStopLoss,OrderTakeProfit(),OrderExpiration());
                           Debug("Buystop80 true - buystoploss is ",buyStopLoss);}
                     ...
                     if (OrderSymbol()==symbol)
                        if (OrderMagicNumber()==magicNumber)
                          ...
                          //  if  (sellStopLoss<OrderOpenPrice())
                             if (sellStopLoss<OrderStopLoss())
                                 {OrderModify(OrderTicket(), OrderOpenPrice(),sellStopLoss,OrderTakeProfit(),OrderExpiration());
                                 Debug("Sell80 true - sellStoploss is ",sellStopLoss);}
        // 90% gain
            ...
            if (OrderMagicNumber()==magicNumber)
               if (OrderType()==ORDER_TYPE_BUY)
                 ...
                 // if (buyStopLoss>OrderOpenPrice())
                   if (buyStopLoss>OrderStopLoss())
                           {OrderModify(OrderTicket(), OrderOpenPrice(),buyStopLoss,OrderTakeProfit(),OrderExpiration());
                           Debug("Buystop90 true - buystoploss is ",buyStopLoss);}
                     ...
                     if (OrderSymbol()==symbol)
                        if (OrderMagicNumber()==magicNumber)
                         ...
                         //   if  (sellStopLoss<OrderOpenPrice())
                             if (sellStopLoss<OrderStopLoss())
                                 {OrderModify(OrderTicket(), OrderOpenPrice(),sellStopLoss,OrderTakeProfit(),OrderExpiration());
                                 Debug("Sell90 true - sellStoploss is ",sellStopLoss);}
