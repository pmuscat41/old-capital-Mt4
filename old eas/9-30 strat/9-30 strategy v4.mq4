...
double MA50= iMA (Symbol(),Period(),inpLMA+20,0,MODE_LWMA,PRICE_CLOSE,0);
      ...
      //manage pending orders
         if ((lastTicket!=-1) && (ticket==lastTicket))
            ...
            if (OrderType()== OP_BUYSTOP)
             if (iClose(Symbol(),Period(),1)<MA50) 
...
/// higher timeframe confirmation- on higher tf:
   //-  Chikuspan < / > price 26 bars back
   //-  above/below green /red cloud
         
if  (longSignal1==true)
      if (iIchimoku(Symbol(), PERIOD_D1,9,26,52,MODE_CHIKOUSPAN,1)>iHigh(NULL,PERIOD_D1,26))
          if (iIchimoku(NULL,PERIOD_D1,9,26,52,MODE_SENKOUSPANB,1)>iIchimoku(NULL,PERIOD_D1,9,26,52,MODE_SENKOUSPANA,1))
             if (iIchimoku(Symbol(), NULL,9,26,52,MODE_CHIKOUSPAN,1)>iHigh(NULL,NULL,26)) 
      longSignal2=true;
         else longSignal2=false;
        
if (shortSignal1==true)
      if (iIchimoku(Symbol(), PERIOD_D1,9,26,52,MODE_CHIKOUSPAN,1)<iLow(NULL,PERIOD_H1,26))
          if (iIchimoku(NULL,NULL,9,26,52,MODE_SENKOUSPANB,1)<iIchimoku(NULL,NULL,9,26,52,MODE_SENKOUSPANA,1))
           if (iIchimoku(Symbol(), PERIOD_D1,9,26,52,MODE_CHIKOUSPAN,1)<iLow(NULL,PERIOD_H1,26))
