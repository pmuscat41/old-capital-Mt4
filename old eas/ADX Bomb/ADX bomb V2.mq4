...
input int     inpLookBack = 2;// lookback in days to switch direction of trade
...
double ADXLookback=  iADX (Symbol(),Period(),inpADXPeriod,PRICE_CLOSE,MODE_MAIN,inpLookBack);
...
if (ticket>0)
   ...
   OrderSelect (ticket,SELECT_BY_TICKET);
       ...
       if ((OrderType()== ORDER_TYPE_BUY)&& (ADXnow<ADXLookback))
            {
             RefreshRates();
        ...
        if ((OrderType()==ORDER_TYPE_SELL) && (ADXnow>ADXLookback))
            {
              RefreshRates();
...
//  Trade entry
      ...
      if ((ADXB4<inpADXTrigger)&&(ADXnow>inpADXTrigger)&&(ADXnow>ADXB4 )&& (ticket==0)&&(iClose(NULL,NULL,1)>iClose(NULL,NULL,2)))  {
