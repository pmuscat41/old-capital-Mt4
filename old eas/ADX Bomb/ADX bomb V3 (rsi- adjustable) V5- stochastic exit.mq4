...
input int      inpADXLookback= 5;//ADX lookback - exit when ADX drops to x candles back
...
double ADXB4= iADX (Symbol(),Period(),inpADXPeriod,PRICE_CLOSE,MODE_MAIN,inpADXLookback);
...
if (ticket>0)
   ...
   OrderSelect (ticket,SELECT_BY_TICKET);
       ...
       if ((OrderType()== ORDER_TYPE_BUY)&& (ADXnow<inpADXLookback))
            {
             RefreshRates();
        ...
        if ((OrderType()==ORDER_TYPE_SELL) && (ADXnow<inpADXLookback))
            {
              RefreshRates();
