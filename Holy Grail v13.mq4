...
input double      inpLots=0.1;// enter lots to trade
...
input string      inpTradeComments     =  "Holy Grail v13";  //Trade comment
...
void waitingForShortEntry ()
 ...
 double  ADX3 = iADX(Symbol(), Period() , 14, PRICE_CLOSE, MODE_MAIN, 1);
   ...
   if (iClose(Symbol(),Period(),1)<TradeEnterPriceShort)
      ...
      TradeOne= orderOpen(ORDER_TYPE_SELL,stplShort,tpShort*inpTPfactor,inpLots,inpTradeComments,inpMagicNumber);
      
      if (inpsecondtrade==true)  TradeTwo= orderOpen(ORDER_TYPE_SELL,stplShort,tpShort,inpLots,inpTradeComments,inpMagicNumber);
