...
// corrected opening errortotalTicket
...
double   inpOrderSize_     ;
 int   inpTotalOrders    =  4; //Set total orders to open
input string   inpTradeComments  =  "Connors TPS  RSI 2 ver 8";  //Trade comment
...
void OnTick()
  {
   ...
   Print("total ticket=  "+ TIK+ "   inpTotal orders=  "+ inpTotalOrders+"       oversold = "+ oversold + "    overbought  = "+ overbought+  "   direction= " + direction + "    lastticket= " +lastTicket +" ticket="+ ticket+ " orderok?= "+ OrderOk);
...
//// first order to open
   ...
   if((TIK==0) && (oversold==true) && (direction>0) && (TIK<inpTotalOrders)  && (OrderOk==true))
     {
      ...
      if((TIK==0) && (overbought==true) &&(direction<0) && (TIK<inpTotalOrders) && (OrderOk==true))
        {
      ...
      for(int i=(OrdersTotal()-1); i>=0; i--)

        {
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==false)
           {
            Print("false result in the loop - loop=" + i + "    total Ticket= " + TIK + "Order lots=  " + OrderLots());
...
void subsequentOrders()

  {
   ...
   if((direction>0) && (TIK<inpTotalOrders)  && (OrderOk==true) && (PreviousPricelong))
     {
      ticket   =  orderOpen(ORDER_TYPE_BUY, inpStopLoss,inpTakeProfit);
      ...
      if((direction<0) && (TIK<inpTotalOrders) && (OrderOk==true) &&(PreviousPriceshort))
        {
         ticket  =  orderOpen(ORDER_TYPE_SELL, inpStopLoss,inpTakeProfit);
...
/// divide by number of orders
   ...
   retOrderSize=inpOrderSize/10;
   
   
Alert ("+++++++++++++++++++++++++++++++++++Order Size in lots=  "+  inpOrderSize);
Alert("----------------------------- Order mini= "+retOrderSize);
