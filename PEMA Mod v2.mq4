...
input string   inpTradeComments  =  "PEMA MOD V2";  //Trade comment
...
void OnTick()  {
 ...
 ///// 
   ...
   Alert ( "  Long Close signal  "+closeLong+  "  previous direction   "+previousDirection+ "ticket ="+ticket+" Lastticket = "+ lastTicket);
   Alert ("Short Close signal  "+closeShort+ " previous direction   "+previousDirection+ "ticket ="+ticket+" Lastticket = "+ lastTicket);
 ...
 int count = 0;
            while ((ticket != -1) && (count < 10))
         {
          ...
          OrderClose (lastTicket,inpOrderSize,Bid,10,Blue);
          if (GetLastError()==0)
                     {
                     ResetLastError();
                     lastTicket=-1;
