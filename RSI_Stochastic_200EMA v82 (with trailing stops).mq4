...
#property strict
input double inpTakeProfit = 0.001; // input take profit
 double      inpStopSteps= 100;// Input one unit of steps for following stop (five steps)
 double      inpStopPoints= 400;// Input points for stoploss
...
input double      inpMultiplier        = 1.2;// input multiplier to increase risk reward
          ...
          double    ATRFactor=             2.5;// enter ATR Factor for trend component
          double   inpPips=0.0005;// enter pips to add to each consecutive trade
input double   enterStop         =  10; // Enter Stop in pips 
...
input string   inpTradeComments  =  "RSI STOCHASTIC 200 V83";  //Trade comment
...
Docket= CheckLongTrade (higherHigh,lowerLow,multiplier,XmarketTurnCounter);}
  ...
  
 {
      ...
      marketTurnCounter=marketTurnCounter+1;
       ...
       double stoploss = NormalizeDouble ( stopvalue+abit+(ATR*Digits),Digits());
     ...
     double  volume = NormalizeDouble(inpOrderSize,Digits());
      ...
      double openPrice = NormalizeDouble (SymbolInfoDouble(Symbol(), SYMBOL_BID), Digits());
       ...
       double takeProfit= NormalizeDouble (inpTakeProfit,Digits());
          ...
          if ( (stomain>stosig) && (XmarketTurnCounter< inpMarketTurnCounter)&& (OpenOrders()<inpMaxOrders)) 
                 {
                       ...
                       if (minstoplevel>spread) stopvalue =minstoplevel*Point; 
                        ...
                        double takeProfit= NormalizeDouble (inpTakeProfit,Digits());
                        double takeProfitPrice =  NormalizeDouble(Bid+inpTakeProfit,Digits());
                        RefreshRates();
