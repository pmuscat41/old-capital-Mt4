...
input double      inpMinRiskReward     =0.5;// Enter min Risk / Reward ratio
...
input string      inpTradeComments     =  "Holy Grail v9";  //Trade comment
...
double GetSwingHigh (int TriggerBar)
...
void waitingForLongEntry ()
 ...
 double static ADX2 = iADX(Symbol(), Period() , 14, PRICE_CLOSE, MODE_MAIN, 1);
   ...
   if (iClose(Symbol(),Period(),1)>TradeEnterPrice && AccountFreeMargin()>inpCashMin)
      ...
      double Lots= NormalizeDouble(LotsToTrade,2);
      PrintFormat ("Lots= "+Lots);
