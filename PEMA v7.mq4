...
void OnTick()  {
   ...
   if (stackedLong==true)
     ...
     double stop=iLow(Symbol(),Period(),2)+MarketInfo (Symbol(),MODE_SPREAD);
      ticket   =  orderOpen(ORDER_TYPE_BUY,stop,inpTakeProfit);
      lastTicket = ticket;
   ...
   Alert ("Spread between Slow and Medium = "+ SSpread);
     ...
     double STP=iHigh (Symbol(),Period(),2)+MarketInfo (Symbol(),MODE_SPREAD)
     ;
         ticket  =  orderOpen(ORDER_TYPE_SELL, STP,inpTakeProfit);
         lastTicket= ticket;
