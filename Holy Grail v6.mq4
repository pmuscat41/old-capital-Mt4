...
input string   inpTradeComments  =  "Holy Grail v6";  //Trade comment
...
double static MAStop= iMA(Symbol(),Period(),30,0,MODE_EMA,PRICE_MEDIAN,1);
double static Mstop =NormalizeDouble(MAStop,Digits);

 
   double static ADX = iADX(Symbol(), Period() , 14, PRICE_CLOSE, MODE_MAIN, 1);  
   double static EMA= iMA(Symbol(),Period(),30,0,MODE_EMA,PRICE_HIGH,1);  
...

{
   ...
   if (TradeTwo>0) OrderSelect(TradeTwo,SELECT_BY_TICKET);
        ...
        { 
        er=OrderModify(TradeTwo,OrderOpenPrice(),MAStop,OrderTakeProfit(),0,clrRed);
...
if (OrderCloseTime()==0)
   ...
   OrderSelect(TradeTwo,SELECT_BY_TICKET,MODE_TRADES);
      if (OrderCloseTime()==0)
      return;
...
void waitingForLongEntry ()
...

  {
   ...
   if (iClose(Symbol(),Period(),1)>TradeEnterPrice && AccountFreeMargin()>inpCashMin)
      ...
      TradeTwo= orderOpen(ORDER_TYPE_BUY,stpl,tp*inpTPfactor,inpOrderSize,inpTradeComments,inpMagicNumber);      
      
      
      if (TradeOne>0 && TradeTwo>0) CurrentCondition=ENUM_CONDITION_IN_A_LONG_HEADING_FOR_TARGET; else 
