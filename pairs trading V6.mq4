...
input bool trade_now=                     False;// Trade immediately
...
input string      inpTradeComments     =  "Pairs Trading v6";  //Trade comment
...
if (trade_now==true && Status==WAITING) CheckEntry ();
      ...
      for(int i=(OrdersTotal()-1); i>=0; i--)
      {
        if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==false)
         ...
         {  OrderSelect(i,SELECT_BY_POS,MODE_TRADES);
               if (OrderMagicNumber()== inpMagicNumber || OrderComment()=="Pairs Trading")
              {
...

        {
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==false)
         ...
         else
           {
            ...
            if(OrderMagicNumber()== inpMagicNumber || OrderComment()=="Pairs Manual")
