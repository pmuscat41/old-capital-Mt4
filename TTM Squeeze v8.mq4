...
input string   inpTradeComments  =  "TTM Squeeze-v8";  //Trade comment
input double   inpMagicNumber    =  00000001; //Magic number
input double   inpSqu =          1.5;//Entr squeeze factor
...
double priceLevel = iClose(Symbol(),Period(),0);


double MA50= iMA(Symbol(),Period(),50,0,MODE_EMA, PRICE_CLOSE,1);

// (MA89>MA89BF)&&
if 
   (priceLevel>MA50) SignalGoLong=1;
   else
   SignalGoLong=0; 
...
// (MA89<MA89BF) &&
  ...
  if 
   (priceLevel<MA50) SignalGoShort=1;
   else
   SignalGoShort=0;  
...
// Keltner Band Calc
      ...
      double MA_Hi = iMA(Symbol(),Period(),14,0,MODE_SMA,PRICE_HIGH,0);
      double MA_Lo = iMA(Symbol(),Period(),14,0,MODE_SMA,PRICE_LOW,0);
      double Kelt_Mid_Band=iMA(Symbol(),Period(),20,0,MODE_SMA,PRICE_TYPICAL,0);
      double KeltnerUpperBand   = Kelt_Mid_Band + ((MA_Hi - MA_Lo)*inpSqu);
      double KeltnerLowerBand = Kelt_Mid_Band - ((MA_Hi - MA_Lo)*inpSqu);
...
////// Get direction
         ...
         if (iMACD(Symbol(),Period(),12,26,9,PRICE_CLOSE,MODE_MAIN,1)>0)direction=2;
