...
input       double StopInPips= 0.000;// enter stop in price
...
input double MaxDays = 10; //Max Days to exit
...
if (ticket>0)

   {
      OrderSelect (ticket,SELECT_BY_TICKET);
       ...
       if ((OrderType()== ORDER_TYPE_BUY)&& (Days>MaxDays))
            {
             RefreshRates();
            OrderClose(ticket,OrderLots(),Bid,10);
            if (GetLastError()==-1)
        ...
        if ((OrderType()==ORDER_TYPE_SELL) && (Days>MaxDays))
